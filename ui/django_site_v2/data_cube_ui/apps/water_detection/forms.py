...
class DataSelectForm(forms.Form):
    ...
    animated_product = forms.ChoiceField(label='Generate Time Series Animation', widget=forms.Select(
        attrs={'class': 'field-long tooltipped'}), help_text='Generate a .gif containing water extents for every scene. Use the output tab to download this result.')
    title = forms.CharField(widget=forms.HiddenInput())
    description = forms.CharField(widget=forms.HiddenInput())

    def __init__(self, result_list=None, animation_list=None, band_list=None, *args, **kwargs):
